<div class="max-w-screen-lg mx-auto p-6 space-y-4 bg-gray-100 border-2 mb-2">
    ...
    @foreach($size as $data)
        <div class="bg-white rounded-lg shadow-lg p-4 flex flex-col items-center w-full">
            ...
            <button class="relative bg-blue-500 hover:bg-blue-600 active:bg-blue-700 text-white py-2 px-4 rounded-md focus:outline-none transition duration-150 ease-in-out group "  onclick="openEditPopup({{ $data->Size_id }}, '{{ $data->Size_name ?? 'null' }}','{{ $data->Size_abb ?? 'null'}}')">
                ...
                <button class="relative bg-red-500 hover:bg-red-600 active:bg-red-700 text-white py-2 px-4 rounded-md focus:outline-none transition duration-150 ease-in-out group " onclick="if(confirm('{{ __('Are you sure you want to delete?') }}')) { window.location.href='size/destroy/{{$data->Size_id}}'; }">
