      ...
      <div class="overflow-x-auto w-full">
        ...
        <tbody>
          ...
          <tr class="{{ $loop->even ? 'bg-gray-100' : 'bg-white' }}">
            ...
            <button class="relative bg-blue-500 hover:bg-blue-600 active:bg-blue-700 text-white py-2 px-4 rounded-md focus:outline-none transition duration-150 ease-in-out group "  onclick="openPositionEditPopup({{ $position->position_id }}, '{{$position->position_name ?? 'null'}}', '{{$position->position_alias ?? 'null'}}')">
                ...
                <button class="relative bg-red-500 hover:bg-red-600 active:bg-red-700 text-white py-2 px-4 rounded-md focus:outline-none transition duration-150 ease-in-out group " onclick="if(confirm('{{ __('Are you sure you want to delete?') }}')) { window.location.href='position/destroy/{{$position->position_id}}'; }">
                ...
                <button class="relative bg-blue-500 hover:bg-blue-600 active:bg-blue-700 text-white py-2 px-4 rounded-md focus:outline-none transition duration-150 ease-in-out group"
                  onclick="toggleActive(this, {{ $position->position_id }})"
                  onmouseover="setHover(this, true)"
                  onmouseout="setHover(this, false)"
                  style="background-color: {{ $position->status === 'Active' ? '#008000' : '#f00' }}; color: white;">
                  <i class="fas {{ $position->status === 'Active' ? 'fa-toggle-on' : 'fa-toggle-off' }}"></i>
