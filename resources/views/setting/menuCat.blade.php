          ...
          <tbody id="inventoryTableBody">
            ...
            <tr class="{{ $loop->index % 2 === 0 ? 'bg-zinc-200' : 'bg-zinc-300' }} text-base {{ $loop->first ? 'border-t-4' : '' }} text-center border-white">
              ...
              <td class="py-3 border border-white">
                ...
                <button class="relative bg-red-500 hover:bg-red-600 active:bg-red-700 text-white py-2 px-4 rounded-md focus:outline-none transition duration-150 ease-in-out group " onclick="if(confirm('{{ __('Are you sure you want to delete?') }}')) { window.location.href='menu_category/destroy/{{$category->Menu_Cate_id}}'; }">
                ...
                <button class="relative bg-blue-500 hover:bg-blue-600 active:bg-blue-700 text-white py-2 px-4 rounded-md focus:outline-none transition duration-150 ease-in-out group"
                  ...
                  onclick="toggleActive(this, {{ $category->Menu_Cate_id }})"
...
function toggleActive(button, materialId) {
    ...
    fetch(`/menu_category/${materialId}/toggle-status`, {
